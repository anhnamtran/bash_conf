...
local function DeleteInactiveBufs()
  ...
  for _, tab in ipairs(vim.api.nvim_list_tabpages()) do
     ...
     visiblebufs = vim.list_extend(visiblebufs, bufs)
  end
  for _, buf in ipairs(vim.api.nvim_list_bufs()) do
    -- The buffer is valid, is not modified, and exists in the buffer list
    if vim.api.nvim_buf_is_loaded(buf) and not vim.api.nvim_buf_get_option(buf, "modified") and not vim.tbl_contains(visiblebufs, buf) then
      if require("lazy.core.config").plugins["vim-bbye"] ~= nil then
